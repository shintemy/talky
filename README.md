...
3. 执行 `./start_talky.command`（或在 Finder 中双击）。
4. 按住热键说话，松开后获取并自动粘贴整理后的文本。

#### 可选：Dock 启动器（.app）

为了更好的“一键启动”体验，可用 Script Editor / Automator 将脚本封装为 `Talky Launcher.app`，再固定到 Dock 并自定义图标。
...
1. 按住热键开始录音。
2. 松开后进行转写（ASR）。
3. 本地大模型进行文本清理与结构化（LLM）。
4. 粘贴到当前聚焦输入框；若无焦点则显示悬浮复制面板。

### 历史记录

每次生成的文本都会追加到当日 Markdown 文件：
